    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6">
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button 
          ...
          className={`w-full sm:w-auto px-4 py-2 ${isRefreshing ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'} text-white text-sm font-medium rounded-lg flex items-center justify-center sm:justify-start`}
        >
          {isRefreshing ? (
            <>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              <div className="font-semibold text-lg truncate">{submission.user.name}</div>
              <div className="text-gray-500 text-sm truncate">{submission.user.email}</div>
            </div>
          </div>
        ...
        <div className="text-gray-600 text-sm mt-3 mb-2">Submitted: {new Date(submission.submittedAt).toLocaleString()}</div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {submission.documents.map((doc) => (
            <div key={doc.id} className="border rounded-lg p-3 sm:p-4 flex flex-col gap-2 bg-gray-50">
              <div className="font-medium truncate">{doc.type}</div>
              <div className="text-gray-500 text-xs truncate">{doc.fileName}</div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className={`flex items-center justify-center px-4 py-2 rounded bg-green-600 text-white font-medium hover:bg-green-700 disabled:opacity-50 ${actionLoading ? 'w-full sm:w-auto' : 'w-full sm:w-auto'}`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center px-4 py-2 rounded bg-red-600 text-white font-medium hover:bg-red-700 disabled:opacity-50 ${actionLoading ? 'w-full sm:w-auto' : 'w-full sm:w-auto'}`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center px-4 py-2 rounded bg-amber-500 text-white font-medium hover:bg-amber-600 disabled:opacity-50 ${actionLoading ? 'w-full sm:w-auto' : 'w-full sm:w-auto'}`}
          >
          ...
          {auditLogs.map((log) => (
            <li key={log.id} className="py-2 text-sm flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                ...
                {log.details && <span className="text-gray-500 ml-2 block md:inline">{log.details}</span>}
              </div>
              <div className="text-gray-500 text-xs mt-1 md:mt-0">{new Date(log.timestamp).toLocaleString()}</div>
            </li>
          ))}
          {auditLogs.length === 0 && (
            <li className="py-4 text-center text-gray-500">No audit logs available</li>
          )}
        </ul>
      </div>
    </div>
